...
@RoutePage()
class AfterSchedulePage extends StatefulWidget {
  const AfterSchedulePage({super.key});

  @override
  State<AfterSchedulePage> createState() => _AfterSchedulePageState();
}

class _AfterSchedulePageState extends State<AfterSchedulePage> {
  String todayDateTime = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  Future _initLoad() async {
    ...
    try {
      ...
      final scheduleModels = await scheduleRepository.fetchAfterSchedules(
          paginationFilter, _searchScheduleTypes, todayDateTime
      );
      if (scheduleModels.isEmpty) {
        paginationFilter.hasNext = false;
      }
      arrangeIconData(scheduleModels);
      _schedules = [];
  ...
  void arrangeIconData(List<ScheduleModel> items) {
      ...
      child: Scaffold(
        body: _isFirstLoading
          ...
          children: [
            ...
            if(!_isFirstLoading && !_isLoadMoreLoading && _schedules.isEmpty)
              Expanded(
              ...
              Expanded(
                child: RefreshIndicator(
                  onRefresh: _initLoad,
                  child: _scheduleListView((!_isFirstLoading && !_isLoadMoreLoading && _schedules.isEmpty))
                ),
              ),
            if(_isFirstLoading)
              Container(
  ...
  Widget _scheduleListView(bool isEmpty){
    print('체크 : $isEmpty');
    return Stack(
        children: [
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              Card(
                ...
                child: ListTile(
                  leading: Icon(_schedules[index].scheduleIconData),
                  title: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(
                        _schedules[index].scheduleTypeName.toString(),
                        style: TextStyle(
                      ...
                      Container(
                        ...
                        child: Text(
                          _schedules[index].startDate.substring(0, 16)
                              .toString(),
                          style: TextStyle(
          ...
          Positioned(
              ...
              child: IconButton.filled(
                  onPressed: (){
                    _showScheduleDialog(true, context, 0);
                  },
  ...
  void _showScheduleAddDialog(BuildContext ctx) {
    ...
    startDateTextEditController = TextEditingController(text: today.substring(0, 10));
    startTimeTextEditController = TextEditingController(text: today.substring(11, 16));
    ...
    Time startTime = Time(hour: int.parse(today.substring(11, 13)), minute: int.parse(today.substring(14, 16)));
    ...
    showDialog(
      ...
      builder: (ctx) {
        dialogContext = ctx;
        final dialogWidth = MediaQuery.of(ctx).size.width*0.7;
        final dialogHeight = MediaQuery.of(ctx).size.height*0.7;
        return Dialog(
          child: Container(
            width: dialogWidth,
            height: dialogHeight,
                          ...
                          Container(
                            ...
                            child: OutlinedButton(
                              onPressed: () => setState(() {
                                ...
                                _schedule.scheduleTypeName = scheduleEnum.name;
                                typeTextEditController.text = scheduleEnum.name;
                              }),
                        ...
                        FilledButton(
                          onPressed: () async {
                            if(typeTextEditController.text.isEmpty){
                              ...
                              _schedule.scheduleDesc = descTextEditController.text;
                              final response = await scheduleRepository.postSchedule(_schedule);
  ...
  void _showScheduleModifyDialog(BuildContext ctx, int index) async {
    try {
      ...
      String? startTimeStr = startDateTime.toString().length < 10 ? "" : startDateTime.toString().substring(0,16);
